...
export default function rescheduleBooking() {
  ...
  const sendNotification = useCallback(() => {
    ...
    } else if (ownerId && !customerId) { // if the appointment is rescheduled by the customer, send a notification to the owner
      sendPushNotification(
        "Appointment Rescheduled",
        "An appointment has been rescheduled",
        {
          url: `/(business)/appointments?appointmentId=${appointmentId}&date=${date.toISOString()}`,
  ...
  }, [customerId, ownerId, sendPushNotification, date, appointmentId])
...
const onSubmit = useCallback(async (data: { date: Date, time: number }) => {
    ...
    sendNotification();
    showToast("Appointment rescheduled", "Your appointment has been moved successfully", "success");
    refetchAvailableSlots();
    reset();
    router.back();
